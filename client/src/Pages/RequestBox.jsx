...
function RequestBox() {
		...
		e.preventDefault();
			try {
				const response = await fetch(`${import.meta.env.VITE_URL}/api/mainDatas/${presentDataId}`, {
					...
					body: JSON.stringify({
						jobRequest: jobDet,
						otherRequest: otherDet,
						yourProducts: proLink
					}),
				});
				if (response.ok) {
  ...
  return (
    ...
    <div className="relative flex flex-col bg-white p-2  text-gray-700 bg-transparent shadow-none rounded-xl bg-clip-border">
  ...
  <form onSubmit={handleSubmit} className="max-w-screen-lg mt-8 mb-2 w-80 sm:w-96">
    <div className="flex flex-col gap-6 mb-1">
      ...
      <div className="relative h-11 w-full min-w-[200px]">
        <input placeholder="Job Details, Eligibility Markers, Salary" onChange={(e)=>setjobDet(e.target.value)} value={jobDet} required
      ...
      <div class="relative h-11 w-full min-w-[200px]">
        <input placeholder="Distributors ,Raw Materials" onChange={(e)=>setyotherDet(e.target.value)} value={otherDet} required
      ...
      <div className="relative h-11 w-full min-w-[200px]">
        <input placeholder="Site URL or Product Links" onChange={(e)=>setproLink(e.target.value)} value={proLink} required
          className="peer h-full w-full rounded-md border border-blue-gray-200 border-t-transparent !border-t-blue-gray-200 bg-transparent px-3 py-3 font-sans text-sm font-normal text-blue-gray-700 outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-blue-gray-200 placeholder-shown:border-t-blue-gray-200 focus:border-2 focus:border-gray-900 focus:border-t-transparent focus:!border-t-gray-900 focus:outline-0 disabled:border-0 disabled:bg-blue-gray-50" />
      </div>
